...
- **Gradient & Glideslope Calculator** — Four modes: (1) basic decimal/percent grade from rise & run with longitudinal (±3%) and transverse (±0.5–3%) limit checks; (2) the 20-ft line-level reduction table (rise in inches per segment → per-segment and cumulative section grade); (3) a full longitudinal profile that takes each 200-ft section grade and checks both the ±3% cumulative and the ±1.5%-per-200-ft change limits at once; (4) glideslope obstacle clearance, giving the run:rise ratio and pass/fail against C-130 (35:1) and C-17 (20:1). Math from SAAZA Unit 1 (slides 64–72); limits from DAFMAN 13-217 Table 4.2. Every result shows the work
