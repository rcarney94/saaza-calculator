...
- **Gradient & Glideslope Calculator** — Five modes: (1) grade ↔ elevation — solve for grade, elevation change, or run, with the rise/drop given in both decimal feet and feet-inches, plus longitudinal (±3%) and transverse (±0.5–3%) limit checks; (2) the 20-ft line-level reduction table (rise in inches per segment → per-segment and cumulative section grade); (3) a full longitudinal profile that checks both the ±3% cumulative and the ±1.5%-per-200-ft change limits at once; (4) a **Zone A / B transverse drop check** that converts the measured transverse grade into the actual elevation drop from the runway edge and checks it against the C-130 limits (Zone A 35′ / 2′9″, Zone B 25′ / 6′3″ cumulative), with custom widths/drops for other airframes; (5) glideslope obstacle clearance giving the run:rise ratio and pass/fail against C-130 (35:1) and C-17 (20:1). Math from SAAZA Unit 1 (slides 64–72); limits from DAFMAN 13-217 Table 4.2. Every result shows the work
