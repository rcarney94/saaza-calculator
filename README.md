...
## Usage
All tools work directly in any mobile or desktop browser, and the suite can be installed to a home screen as an app (see below). Works offline once loaded.
...
## Install & offline (PWA)
The suite is a Progressive Web App, so it can be installed to a phone's home screen and run full-screen like an app (Android: "Add to Home screen" / Install; iOS Safari: Share → "Add to Home Screen"). A service worker caches every tool on first load, so the whole suite works with no signal once it has been opened online once. Reference PDFs and slide decks are cached the first time each one is opened.

Updates are controlled, not automatic: when a new version is published, anyone online sees a banner offering to update, with a warning to wait for a strong connection before doing so; it only updates when they tap **Update now**. The footer shows the running version and a **Check for updates** button.

### Publishing an update (version-bump checklist)
1. Edit/replace whatever tool files changed and upload them to the repo.
2. In **`sw.js`**, bump the `VERSION` string (e.g. `2026.06.21` → `2026.07.02`). This is what triggers the update banner for everyone.
3. **If you added or renamed any file**, update the `SHELL` list in `sw.js` to match (names are case-sensitive). A missing/misnamed entry won't break the install — it just won't be cached offline.
4. Upload `sw.js` (and `index.html` if it changed).
5. Test: open the site online, confirm the **Update now** banner appears, tap it, and confirm it reloads to the new version shown in the footer.

## File Structure
- Root: HTML tools, flashcard decks, and the Team Lead Checklist Excel file
- Root (PWA): `manifest.json`, `sw.js` (service worker), `icon-192.png`, `icon-512.png` — all must sit in the repo root alongside `index.html`
